...
namespace Serilog
{
    ...
    public static class LoggerConfigurationMSSqlServerExtensions
    {
            ...
            )
        {
            ...
            MSSqlServerConfigurationSection serviceConfigSection = null;
            //  need to implement passing the IConfigurationSection to the sink from SerilogConfiguration
        ...
        private static string GetConnectionString(string nameOrConnectionString)
        {
            ...
            // If there are no `=`, we throw exception as this is not implemented yet
            if (nameOrConnectionString.IndexOf('=') < 0)
            {
                SelfLog.WriteLine("MSSqlServer sink configured value {0} is reference to connection string in configuration which is not supported in ths version. Only raw connection string is supported.", nameOrConnectionString);
                throw new ArgumentException("Reference to connection string in other ConfigurationSection is not allowed");
            }
        ...
        {
            foreach (ColumnConfig c in serviceConfigSection.Columns.GetColumnsConfigs())
            {
                // Set the type based on the defined SQL type from config
                LogColumn column = new LogColumn(c.ColumnName);
                ...
                {
                    columnOptions.AdditionalDataColumns = new Collection<LogColumn>();
                }
